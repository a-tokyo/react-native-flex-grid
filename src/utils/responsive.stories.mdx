...
A set of useful utilities for responsive design.
...
- example:
  ...
  mediaQuery({ up: 'sm', { width: 20 } }); // `{ width: 20 }` or `null`
  mediaQuery({ down: 'md', { width: 20 } }); // `{ width: 20 }` or `null`
  mediaQuery({ exact: 'lg', { width: 20 } }); // `{ width: 20 }` or `null`
  ```